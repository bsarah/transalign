...
genParseTabularBlast = do
  ...
  return $ BlastTabularResult (B.pack ("BLAST" ++ _blastProgram)) (toLB $ C.pack _blastQueryId) (toLB $ C.pack _blastQueryName) (toLB $ C.pack _blastDatabase) _blastHitNumber (V.fromList _tabularHit)
